...
extern Ble_scanRsp_t m_bls_scan_rsp[10];
extern uint8_t rspNum;
bool NullTxFrame = false;
uint8_t send_scan_rsp_index = 0;
      ...
      case MLME_JOIN:
        {
            if( mlmeConfirm->Status == LORAMAC_EVENT_INFO_STATUS_OK )
            {
                ...
                Write_OLED_string("OTAA Success");
                // Switch LED 1 ON
                //GpioWrite( &Led1, 0 );
                //TimerStart( &Led1Timer );
            }
      ...
      case DEVICE_STATE_SEND:
        {
            if( NextTx == true )
            {
                ...
                {
                    rspNum = 0;
                    send_scan_rsp_index = 0;
                    Write_OLED_string("Send Success");
                }
            }
            if( ComplianceTest.Running == true )
            {
