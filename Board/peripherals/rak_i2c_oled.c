...
int oled_twi_init()
{
    ...
    const nrf_drv_twi_config_t twi_lis_config = {
        .scl                = OLED_TWI_SCL_PIN,
        .sda                = OLED_TWI_SDA_PIN,
    ...
    {
        NRF_LOG_INFO("rak_i2c_init ret %d\r\n", err_code);
        return err_code;
    }
...
{
    uint32_t err_code; 
    err_code = rak_i2c_write(OLED_ADDR, 0x00, &I2C_Command, 1);
    if(err_code != NRF_SUCCESS)
    {
        NRF_LOG_INFO( "rak_i2c_write err %d\n", err_code);
    }
}

void WriteDat(unsigned char I2C_Data)//写数据
{
    rak_i2c_write(OLED_ADDR, 0x40, &I2C_Data, 1);
}
